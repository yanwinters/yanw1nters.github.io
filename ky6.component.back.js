(function () {
  const backcnv = document.querySelector('#backcnv');

  const backctx = backcnv.getContext('2d');

  let cw , ch, cx, cy;

  function resizeCanvas() {
    cw = document.getElementById('backcnv').clientWidth;
    ch = document.getElementById('backcnv').clientHeight;
    cx = cw / 2;
    cy = ch / 2.5;
  }
  ...
  function drawRect(color, x, y, w, h, shadowColor, shadowBlur) {
    backctx.shadowColor = shadowColor || 'black';
    backctx.shadowBlur = shadowBlur || 1;
