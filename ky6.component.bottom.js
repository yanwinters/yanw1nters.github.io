(function () {
  const bottomcnv = document.querySelector('#bottomcnv');

  const bottomctx = bottomcnv.getContext('2d');

  let cw , ch, cx, cy;

  function resizeCanvas() {
    cw = document.getElementById('bottomcnv').clientWidth;
    ch = document.getElementById('bottomcnv').clientHeight;
    cx = cw / 2;
    cy = ch / 2.6;
  }
  ...
  function drawRect(color, x, y, w, h, shadowColor, shadowBlur) {
    bottomctx.shadowColor = shadowColor || 'black';
    bottomctx.shadowBlur = shadowBlur || 1;
    bottomctx.fillStyle = color;
    bottomctx.fillRect(x, y, w, h);
  }
