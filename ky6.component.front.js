(function () {
  const frontcnv = document.querySelector('#frontcnv');

  const frontctx = frontcnv.getContext('2d');

  let cw , ch, cx, cy;

  function resizeCanvas() {
    cw = document.getElementById('frontcnv').clientWidth;
    ch = document.getElementById('frontcnv').clientHeight;
    cx = cw / 2;
    cy = ch / 2.5;
  }
  ...
  function drawRect(color, x, y, w, h, shadowColor, shadowBlur) {
    frontctx.shadowColor = shadowColor || 'black';
    frontctx.shadowBlur = shadowBlur || 1;
    frontctx.fillStyle = color;
    frontctx.fillRect(x, y, w, h);
  }
