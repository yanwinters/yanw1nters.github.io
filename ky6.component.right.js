(function () {
  const rightcnv = document.querySelector('#rightcnv');

  const rightctx = rightcnv.getContext('2d');

  let cw , ch, cx, cy;

  function resizeCanvas() {
    cw = document.getElementById('rightcnv').clientWidth;
    ch = document.getElementById('rightcnv').clientHeight;
    cx = cw / 2;
    cy = ch / 2.6;
  }
  ...
  function drawRect(color, x, y, w, h, shadowColor, shadowBlur) {
    rightctx.shadowColor = shadowColor || 'black';
    rightctx.shadowBlur = shadowBlur || 1;
